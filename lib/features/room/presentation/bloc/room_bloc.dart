...
class RoomBloc extends Bloc<RoomEvent, RoomState>{
  ...
  Future<void> _onJoinRoom(JoinRoomEvent event, Emitter<RoomState> emit) async{
    ...
    try{
      ...
      // 1. Join the room
      await roomRepository.joinRoom(event.roomId, event.userId);
  ...
  Future<void> _onLeaveRoom(LeaveRoomEvent event, Emitter<RoomState> emit) async{
    ...
    try{
      ...
      // 1. Leave the room
      await roomRepository.leaveRoom(event.roomId, event.userId);
  ...
  Future<void> _onFetchAvailableRooms(FetchAvailableRoomsEvent event, Emitter<RoomState> emit) async{
    emit(RoomState.loading());
    try{
